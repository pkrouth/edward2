...
class ExpectedCalibrationError(tf.keras.metrics.Metric):
  ...
  def update_state(self, labels, probabilities, **kwargs):
    ...
    probabilities = tf.reshape(tf.convert_to_tensor(probabilities, self.dtype),
                               tf.concat([tf.shape(labels), [-1]], axis=0))

    # Extend any probabilities of shape [N, 1] to shape [N, 2].
    # NOTE: XLA does not allow one to use different shapes in the branches of a
    # conditional statement. Therefore, we resort to the following approach.
    k = tf.math.maximum(2, tf.shape(probabilities)[-1])
    probabilities = tf.concat([1. - probabilities, probabilities], axis=-1)
    probabilities = probabilities[..., -k:]  # [N, k]
